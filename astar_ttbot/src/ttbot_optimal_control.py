...
def pFilter(k, Ns, w, particles, mu, sigma, h, zk, uk,T, x_curr, y_curr, v):
  ...
  for i in range(Ns):
    ...
    res = sys(k, np.array(xktemp).reshape(1,2), uk, gen_sys_noise(sigma), h, v)
...
def optimal_control_loop(x_curr, y_curr, x_past, y_past, grid_width, grid_length, speed, angles_past):
  ...
  while (k<T):
    ...
    [xhTemp, finalPFTemp] = pFilter(k, finalPF[k-1][1], finalPF[k-1][2], finalPF[k-1][3], mu, sigma, h, z_measurements[k-1], angle_inputs[k-1],T, x_curr, y_curr, v) #sys, z_measurements[k], finalPF, angle_inputs[k], x[k], h)
  ...
  optimal_angle = optimize.fminbound(DFIM, -3.14, 3.14,  args = (sRes,r,h,v,m,lambd))

#  print("Optimal Angle Inside: ", optimal_angle)
...
def callback(data):
    ...
    print("X_current: ", x_curr, "Y_current: ", y_curr)
    speed = 0.2
    grid_width = 544
    grid_length = 512
    ...
    global x_past
    x_past.append(x_curr)

    global y_past
    y_past.append(y_curr)


    global angles_past 
    angles_past.append(yaw)


    global action_angle
    if (len(angles_past)>100 and len(x_past)>100 and len(y_past)>100):

        angles_past = angles_past[-100:]
        x_past = x_past[-100:]
        y_past = y_past[-100:]

        action_angle = optimal_control_loop(x_curr, y_curr, x_past, y_past, grid_width, grid_length, speed, angles_past)
...
def callback2(data):
 #   print("CALLBACK 2")
    ...
    avgDistance = list(data.ranges)
    avgDistance = avgDistance[250:450]

    sum = 0
    for i in avgDistance:
        if (i > 0):
            sum += i
    avgDistance = sum/200

    #Distance should be more than 0.6
    move.linear.x = 0.2
    move.angular.z = action_angle*1000000

    stop.linear.x = 0
    stop.angular.z = 1

#    print("Action Action: ", action_angle)
    if (avgDistance >= 0.8):
        rospy.loginfo("Moving Forward")
        print(move)
        for x in range(0,5):
            pubObject.publish(move)
#            r.sleep()        
    else:
        rospy.loginfo("Avoiding Obstacle")
        print(stop)
        for x in range(0,10):
            pubObject.publish(stop)
...
rospy.init_node('Optimal_control', anonymous=False)
...
pubObject = rospy.Publisher('cmd_vel_mux/input/navi', Twist, queue_size=10)
